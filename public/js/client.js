...
// lobbySpectators = [];
playerSlotHTML = [];
let roomOwner = null; // Thêm biến lưu chủ phòng
...
// Thêm hàm kickPlayer để xử lý việc kick người chơi khác
function kickPlayer(kickedClientId) {
  const payLoad = {
    method: "kickPlayer",
...
// Thêm hàm transferOwnership để chuyển quyền chủ phòng
...
// Thêm hàm để tạo popup cho việc kick người chơi
function showKickPopup(targetClientId, playerName) {
  // Tạo popup
  const kickPopup = document.createElement('div');
  kickPopup.className = 'kick-popup';
  kickPopup.innerHTML = `
    <div class="kick-popup-content">
      <h3>Kick người chơi</h3>
      <p>Bạn có chắc chắn muốn kick người chơi ${playerName}?</p>
      <div class="kick-popup-buttons">
        <button id="confirm-kick">Đồng ý</button>
        <button id="cancel-kick">Hủy</button>
      </div>
    </div>
  `;
  
  document.body.appendChild(kickPopup);
  
  // Xử lý các sự kiện nút
  document.getElementById('confirm-kick').addEventListener('click', function() {
    kickPlayer(targetClientId);
    document.body.removeChild(kickPopup);
  });
...
// Thêm hàm để tạo popup cho việc chuyển quyền chủ phòng
function showTransferOwnershipPopup(targetClientId, playerName) {
  // Tạo popup
  const transferPopup = document.createElement('div');
  transferPopup.className = 'kick-popup'; // Sử dụng cùng style với kick-popup
  transferPopup.innerHTML = `
    <div class="kick-popup-content">
      <h3>Chuyển quyền chủ phòng</h3>
      <p>Bạn có chắc chắn muốn chuyển quyền chủ phòng cho ${playerName}?</p>
      <div class="kick-popup-buttons">
        <button id="confirm-transfer">Đồng ý</button>
        <button id="cancel-transfer">Hủy</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('confirm-transfer').addEventListener('click', function() {
    transferOwnership(targetClientId);
    document.body.removeChild(transferPopup);
  });
...
// Thêm hàm để tạo menu tùy chọn khi chủ phòng click vào người chơi
function showPlayerOptions(event, targetClientId, playerName) {
  event.preventDefault();
  event.stopPropagation();
  
  // Kiểm tra nếu đã có menu thì xóa đi
  const existingMenu = document.querySelector('.player-options-menu');
  if (existingMenu) {
  ...
  optionsMenu.className = 'player-options-menu';
  optionsMenu.style.position = 'absolute';
  optionsMenu.style.left = `${event.pageX}px`;
  optionsMenu.style.top = `${event.pageY}px`;
  
  optionsMenu.innerHTML = `
    <div class="option-item kick-option">Kick người chơi</div>
    <div class="option-item transfer-option">Chuyển quyền chủ phòng</div>
  `;
  
  document.body.appendChild(optionsMenu);
  
  // Xử lý sự kiện kick
  document.querySelector('.kick-option').addEventListener('click', function() {
    document.body.removeChild(optionsMenu);
    showKickPopup(targetClientId, playerName);
  });
  
  // Xử lý sự kiện chuyển quyền chủ phòng
  document.querySelector('.transfer-option').addEventListener('click', function() {
    document.body.removeChild(optionsMenu);
    showTransferOwnershipPopup(targetClientId, playerName);
  });
...
// Hàm cập nhật giao diện người dùng để hiển thị chủ phòng
function updateRoomOwnerUI() {
  // Xóa tất cả các crown icon hiện có
  document.querySelectorAll('.crown-icon').forEach(icon => {
    icon.remove();
  });
  
  // Thêm crown icon cho chủ phòng trong danh sách người chơi
  document.querySelectorAll('.users-list-box').forEach((box, index) => {
    if (spectators[index] && spectators[index].clientId === roomOwner) {
      const crownIcon = document.createElement('i');
      crownIcon.className = 'fas fa-crown crown-icon';
      box.querySelector('.users-list-info').prepend(crownIcon);
      
      // Thêm style cho người chơi là chủ phòng
      box.classList.add('room-owner');
    } else {
      box.classList.remove('room-owner');
    }
    
    // Nếu người dùng hiện tại là chủ phòng và đang xem người chơi khác, hiển thị tùy chọn
    if (clientId === roomOwner && spectators[index] && spectators[index].clientId !== clientId) {
      box.classList.add('can-kick');
      box.setAttribute('data-client-id', spectators[index].clientId);
      box.setAttribute('data-nickname', spectators[index].nickname);
      
      // Thêm sự kiện click hoặc right click
      box.addEventListener('contextmenu', function(e) {
        showPlayerOptions(e, this.getAttribute('data-client-id'), this.getAttribute('data-nickname'));
      });
    }
  });
  
  // Hiển thị các nút điều khiển của chủ phòng nếu người dùng hiện tại là chủ phòng

}

// Hàm hiển thị bảng quản lý phòng
function showRoomManagementPanel() {
  // Tạo panel
  const panel = document.createElement('div');
  panel.className = 'room-management-panel';
  panel.innerHTML = `
    ...
    <div class="panel-content">
      <div class="player-list-container">
        ...
        <ul class="management-player-list">
          ${generatePlayerList()}
        </ul>
      </div>
    </div>
  ...
  // Thêm các sự kiện cho các nút trong danh sách người chơi
  document.querySelectorAll('.kick-player-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      const targetId = this.getAttribute('data-client-id');
      const targetName = this.getAttribute('data-nickname');
      document.body.removeChild(panel);
      showKickPopup(targetId, targetName);
    });
  });
  ...
  document.querySelectorAll('.transfer-owner-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      const targetName = this.getAttribute('data-nickname');
      document.body.removeChild(panel);
      showTransferOwnershipPopup(targetId, targetName);
    });
  });
...
// Hàm tạo danh sách người chơi cho bảng quản lý
function generatePlayerList() {
  ...
  spectators.forEach(spec => {
    if (spec.clientId !== clientId) {
      html += `
        <li class="management-player-item">
          <div class="player-info">
            <img src="/imgs/avatars/${spec.avatar}.svg" alt="avatar" class="player-avatar-small">
            <span>${spec.nickname}</span>
          </div>
          <div class="player-actions">
            <button class="kick-player-btn" data-client-id="${spec.clientId}" data-nickname="${spec.nickname}">
              <i class="fas fa-times"></i> Kick
            </button>
            <button class="transfer-owner-btn" data-client-id="${spec.clientId}" data-nickname="${spec.nickname}">
              <i class="fas fa-crown"></i> Chuyển quyền
            </button>
          </div>
        </li>
      `;
  ...
  if (html === '') {
    html = '<li class="no-players">Không có người chơi khác trong phòng</li>';
  }
...
ws.onmessage = (message) => {
  ...
  // Xử lý sự kiện khi bị kick
  if (response.method === "youWereKicked") {
    alert("Bạn đã bị chủ phòng kick.");
    // Chuyển hướng về trang chủ
    window.location.href = "/";
  }
  
  // Xử lý sự kiện khi có người bị kick
  if (response.method === "playerKicked") {
    const kickedClientId = response.kickedClientId;
    const kickedPlayerName = response.kickedPlayerName;
    
    // Thông báo cho người chơi khác
    alert(`Người chơi ${kickedPlayerName} đã bị kick khỏi phòng.`);
    
    // Cập nhật giao diện nếu cần
    const kickedPlayerElement = document.querySelector(`[data-client-id="${kickedClientId}"]`);
    if (kickedPlayerElement) {
      const parentList = kickedPlayerElement.parentElement;
      parentList.removeChild(kickedPlayerElement);
    }
  }
  
  // Xử lý sự kiện khi quyền chủ phòng được chuyển
  if (response.method === "ownershipTransferred") {
    const newOwnerId = response.newOwnerId;
    const newOwnerName = response.newOwnerName;
    const previousOwnerLeft = response.previousOwnerLeft || false;
    
    roomOwner = newOwnerId;
    
    // Cập nhật trạng thái của client nếu họ là chủ phòng mới
    if (clientId === newOwnerId) {
      theClient.isRoomOwner = true;
      
      if (previousOwnerLeft) {
        alert(`Chủ phòng đã rời đi. Bạn đã trở thành chủ phòng mới.`);
      } else {
        alert(`Bạn đã được chuyển quyền chủ phòng.`);
      }
    } else {
      theClient.isRoomOwner = false;
      
      if (previousOwnerLeft) {
        alert(`Chủ phòng đã rời đi. ${newOwnerName} đã trở thành chủ phòng mới.`);
      } else {
        alert(`${newOwnerName} đã trở thành chủ phòng mới.`);
      }
    }
  ...
  if (response.method === "create") {
    ...
    roomOwner = response.game.roomOwner; // Cập nhật roomOwner
  ...
  if (response.method === "join") {
    ...
    playerSlotHTML = response.playerSlotHTML;
    roomId = response.roomId;
    roomOwner = response.roomOwner; // Cập nhật roomOwner
  ...
  if (response.method === "joinClient") {
    ...
    // Cập nhật giao diện chủ phòng sau khi thêm xong người chơi
    updateRoomOwnerUI();
  }
  ...
  if (response.method === "updateClientArray") {
    ...
    // Cập nhật giao diện chủ phòng khi có người chơi mới tham gia
    updateRoomOwnerUI();
  }
  ...
  if (response.method === "joinMidGameUpdate") {
    ...
    // Cập nhật giao diện chủ phòng khi có người mới tham gia trong game
    updateRoomOwnerUI();
  }
  ...
  // Xử lý phản hồi khi kiểm tra route và tìm thấy phòng hiện có
  if (response.method === "roomExists") {
    console.log("Phòng tồn tại: ", response.roomId);
    gameId = response.gameId;
    roomId = response.roomId;
    
    if (response.existingGame && response.existingGame.roomOwner) {
      roomOwner = response.existingGame.roomOwner;
    }

    // Tiếp tục với quy trình tham gia phòng
    $("#loading-screen").removeClass("hide-element");
    const payLoadLength = {
      method: "playersLength",
      gameId: gameId,
    ...
    // Đặt độ trễ 50ms để phương thức trên phản hồi trước khi hàm dưới đây bắt đầu
    setTimeout(function () {
      if (playersLength >= 7) {
        $("#loading-screen").addClass("hide-element");
        alert("Phòng đã đầy!");
  ...
  // Xử lý phản hồi khi kiểm tra route và không tìm thấy phòng
